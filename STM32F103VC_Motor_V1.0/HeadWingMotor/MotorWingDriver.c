...
static void ExecSpeedPWM(MOTOR_WING_PARM* motor_W, int16_t PWM);
/****************************************************
函数名:MotorWingInit
功能: 翅膀电机实体初始化
...
int8_t _Motor_Bottom(MOTOR_WING_PARM* motor_WL,MOTOR_WING_PARM* motor_WR, uint16_t speed){
	...
	motor_WL->ExecSpeedPWM(motor_WL, speed);
	motor_WR->ExecSpeedPWM(motor_WR, speed); 
...
void _Motor_TryUP(MOTOR_WING_PARM* motor_WL,MOTOR_WING_PARM* motor_WR){
	...
	do{
		motor_WL->ExecSpeedPWM(motor_WL, motor_WL->PWM_Base);
		if(Finish_L == 0)
		...
		motor_WR->ExecSpeedPWM(motor_WR, motor_WR->PWM_Base);
		if(Finish_R == 0)
...
int8_t _Motor_Top(MOTOR_WING_PARM* motor_WL,MOTOR_WING_PARM* motor_WR){
	...
	HAL_GPIO_WritePin(motor_WL->GPIOx, motor_WL->GPIO_Pin, (GPIO_PinState)DIR_WING_U);
	HAL_GPIO_WritePin(motor_WR->GPIOx, motor_WR->GPIO_Pin, (GPIO_PinState)DIR_WING_U);
	motor_WL->ExecSpeedPWM(motor_WL, WING_INIT_PWM);
	motor_WR->ExecSpeedPWM(motor_WR, WING_INIT_PWM);
	motor_WL->EncoderLast = motor_WL->Encoder;
	motor_WR->EncoderLast = motor_WR->Encoder;
	do{
		DelayMS(300);
...
功能: 设置输出PWM
****************************************************/	 
void ExecSpeedPWM(MOTOR_WING_PARM* motor_W, int16_t PWM){
	if(PWM > WING_MAX_PWM) PWM = WING_MAX_PWM;
	if(PWM < 0) PWM = 0;
  if(motor_W->MotorID == ID_MOTOR_LC)
    motor_W->TIMx->CCR2 = PWM;
  else if(motor_W->MotorID == ID_MOTOR_RC)
    motor_W->TIMx->CCR1 = PWM;
}
...
void SetWingMotorParam(MOTOR_WING_PARM* motor, uint16_t speed, int16_t angle){
	...
	motor->EncoderCMD = (int16_t)(offset_angle * motor->Angle_Code);		 //计算角度偏差对应编码值
...
void ExecuteWingMotor(MOTOR_WING_PARM* motor){
	...
	motor->ExecSpeedPWM(motor, motor->Speed);//执行电机速度
}
...
void StopWingMotor(MOTOR_WING_PARM* motor){
	...
	motor->ExecSpeedPWM(motor, 0);//设置电机速度为0
	motor->Encoder = 0;
}
...
void KeepWingMotor(MOTOR_WING_PARM* motor){
	...
	HAL_GPIO_WritePin(motor->GPIOx, motor->GPIO_Pin, (GPIO_PinState)DIR_WING_U);
	motor->ExecSpeedPWM(motor, WING_KEEP_PWM);				  //执行电机速度
...
void ScanWingMotorLimit(void){
	...
	if(Motor_WL.Down_Flag == 1){
		Motor_WL.Down_Flag ++;
		Motor_WL.ExecSpeedPWM(&Motor_WL, WING_DOWN_PWM); 
	}else if(Motor_WL.Down_Flag == 2){
		Motor_WL.Down_Flag = 0;
		Motor_WL.ExecSpeedPWM(&Motor_WL, 0);
	}
	
	if(Motor_WR.Down_Flag == 1){
		Motor_WR.Down_Flag ++;
		Motor_WR.ExecSpeedPWM(&Motor_WR, WING_DOWN_PWM);
	}else if(Motor_WR.Down_Flag == 2){
		Motor_WR.Down_Flag = 0;
		Motor_WR.ExecSpeedPWM(&Motor_WR, 0);
	}
	//左翅限位检测
