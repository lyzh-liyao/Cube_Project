...
****************************************************/
 void SetHeadMotorParam(MOTOR_HEAD_PARM* motor, uint16_t speed,uint16_t angle){
	...
	int16_t Diff_Angle = motor->Angle_Cur - angle;
	...
	motor->SpeedCMD = speed / motor->PID_Proportion;//speed / PID_PROPORTION;
	...
	if(angle == 120 &&  Motor_H.BoardVersion == 45){//中点不通过计算直接定值
		...
		motor->Angle_Enc_CMD = angle * motor->Angle_Enc_Code;
	}
...
//速度为100时 提前70个码开始线性减速比较合适  故从速度*0.7个码开始减速
	if(Diff_Angle_Enc < (Motor_H.SpeedCMD*3) )
			Motor_H.Speed = Motor_H.SpeedCMD * (Diff_Angle_Enc/(Motor_H.SpeedCMD*3.0));
